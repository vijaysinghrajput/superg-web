...
export const OrderDetails = ({ data }) => {
  ...
  const cancelMyOrder = async (order_number) => {
    ...
    const data = await cancelOrder(
      order_number,
      reasionForCancelingTheOrders[canceledResion - 1].reasion
    );
    if (data) {
    ...
    <>
      ...
      >
        <Flex justifyContent={"space-between"}>
          ...
          <Box>
            {Order_status === "Cancel" ? (
              <Text color="red" fontSize={12} fontWeight="600">
                Canceled
              </Text>
            ) : Order_status === "Delivered" ? (
              <Text color="#00b778" fontSize={12} fontWeight="600">
                Delivered
              </Text>
            ) : (
              <Text color="#ffb116" fontSize={12} fontWeight="600">
                {Order_status}
              </Text>
            )}
          </Box>
        ...
        <Box>
          ...
          <Box>
            <Text color="#000" fontWeight={"600"} fontSize={12}>
              {address_details?.name}, {address_details?.phone}
            </Text>
            <Text color="#000" fontWeight={"600"} fontSize={12}>
              {address_details?.user_house_no}, {address_details?.address},{" "}
              {address_details?.base_address}
            </Text>
          </Box>
        </Box>
        <Box mt={3}>
              ...
              return (
                ...
                >
                  <Flex alignItems={"center"}>
                    <Image
                      height={8}
                      boxShadow="0 1px 5px 0 #d7d7d7"
                      borderRadius={10}
                      src={URL + "/images/product-images/" + item.p_image}
                    />
                    <Box>
                      <Text ml={3} fontWeight="600" color={"#000"}>
                        {item.p_name} x{item.p_qty}
                      </Text>
                      <Text ml={3} fontSize={10} color={"#686868"}>
                        {item.p_size}
                        {item.p_unit}
                      </Text>
                    </Box>
                  ...
                  <Text fontWeight={"600"}>
                    ₹{" "}
                    {Math.round(
                      item.p_price - item.p_price * (item.p_discount / 100)
                    ) * item.p_qty}
                  </Text>
                </Flex>
              );
        ...
        <Box mt={4}>
          <Flex justifyContent={"space-between"} mb={2}>
            ...
            <Text style={{ fontWeight: "700" }}>
              ...
              {Math.round(
                data.isDevApplied
                  ? data.total_amount - data.delcharge
                  : data.total_amount
              )}
            </Text>
          </Flex>
          {Number(data.isDevApplied) ? (
            <Flex justifyContent={"space-between"} mb={2}>
              <Text fontSize={12}>Delivery Charges</Text>
              <Text style={{ fontWeight: "700" }}>
                +₹ {data.isDevApplied ? data.delcharge : "00"}
              </Text>
            </Flex>
          ) : null}
          <Divider width={"80%"} margin="auto" my={3} />
          <Flex justifyContent={"space-between"} alignItems="center" mb={2}>
            <Text fontSize={16} style={{ fontWeight: "700" }}>
              Total
            </Text>
            <Text style={{ fontWeight: "700" }}>
              ₹{" "}
              {Number(data.is_carry_bag_taken)
                ? Math.round(data.total_amount) + Number(data.carry_bag_charge)
      ...
      <Modal onClose={onCloseModal} isOpen={isOpenModal} size={"xs"} isCentered>
        ...
        <ModalContent>
          ...
          <ModalFooter justifyContent={"end"}>
            <Button
              isLoading={isLoading}
              disabled={!canceledResion}
              colorScheme={"red"}
              size={"sm"}
              onClick={() => cancelMyOrder(data.order_number)}
            >
              Cancel Order
            </Button>
