...
const ProductsBySubcategory = (props) => {
  ...
  const { parent_id } = subcategories.find((sc) => sc.id == subcatID);
  const cat = categories.find((c) => c.id == parent_id);
  ...
  useEffect(() => {
    ...
    setSubCat(subcategories.filter((sc) => sc.parent_id == parent_id));
    setIsLoading(false);
  }, [products, subcatID]);
          ...
          >
            <Box pb={20}>
              <Link
                ...
                to={
                  "/" +
                  (cat.name + " delivery in gorakhpur")
                    .replace(/\s/g, "-")
                    .toLowerCase() +
                  "/" +
                  cat.id +
