...
export default function LoginDrawer({ isLoginOpen, onLoginOpen, onLoginClose }) {

    const { setUserLogin } = useContext(ContextData);
    const { setMobile, setOtpSent, otpSent, setOtpFromServer, wrongOTP, setInputOTP, inputOTP, wrngOTP, otpFromServer, mobile, setLoading, isLoading, setIsLogedIn, isLogedIn } = useAuthentication();
    ...
    return (
        <>
            ...
            <Drawer placement={"bottom"} onClose={onLoginClose} isOpen={isLoginOpen} disabled={true}>
                <DrawerOverlay />
                <DrawerContent
                    borderTopRadius={12}
                >
                    <DrawerHeader borderBottomWidth='0px' textAlign={"center"}>Sign In</DrawerHeader>
                    <DrawerCloseButton />
                    <DrawerBody>
                        {otpSent ? <Box>
                            <Box mb={5} display="flex" justifyContent={"center"}>
                                <Text textAlign={"center"} mr={2}>OTP send to {mobile}</Text>
                                <AiFillEdit color='#ca0808' size={18} onClick={() => setOtpSent(false)} />
                            </Box>
                            <OtpInput
                                value={inputOTP}
                            ...
                            <Box my={3} display="flex" justifyContent={"end"}>
                                <Button disabled={counter} bg="none" size={"xs"} onClick={resendOTP} color={"#ca0808"}>SEND AGAIN {counter != 0 ? counter : ""}</Button>
                            </Box>
