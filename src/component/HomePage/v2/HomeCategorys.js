    ...
    return (
        <>
            <div class="osahan-categories">
                <Text fontSize={20} fontWeight="800" my={4}>Shop by categorys</Text>
                <div class="row">
                    {!subcategories ? (
                        <>
                            <CategoryLoading />
                            ...
                            {subcategories.map((item, i) => {
                                return (
                                    <div class="col-4 col-md-2 mb-0 p-1" key={i}>
                                        <Box bg="#cdeeff" className="list-card h-100 rounded overflow-hidden position-relative shadow-sm">
                                            <Flex p={3} justifyContent="center" alignItems={"center"} h="100%">
                                                <Link state={location.pathname} to={"/" + (item.name + " delivery in gorakhpur").replace(/\s/g, "-").toLowerCase() + "/" + item.id + "/" + item.name}>
                                                    <Text fontWeight={"600"} fontSize={12} color="#000" textAlign="center">{item.name}</Text>
