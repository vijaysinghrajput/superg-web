...
export default function AddFullAddress({ isOpen, onClose, address, position, onCloseMain, setUserAddressData, type, editData }) {
    ...
    const saveMyAddress = async () => {
        ...
        isValid.then((val) => {
            ...
            const { UserID, addressType, fullName, mobile, houseNo, addressFromUser, user_city, user_full_address, state, lat, lng } = dataToSaveAddress;
        ...
        <>
            <Drawer placement={"bottom"} onClose={onClose} isOpen={isOpen}>
                ...
                <DrawerContent
                    borderTopRadius={12}
                >
                    <DrawerHeader borderBottomWidth='1px'>Enter full information</DrawerHeader>
                    <DrawerBody>
                        ...
                        <Box my={2}>
                            <FormControl mt={1} id="email">
                                ...
                                <Input
                                    ...
                                    type="text"
                                    onChange={e => setInfo(rest => {
                                        return { ...rest, fullName: e.target.value }
                                    })}
                                    isInvalid={errorFeald === "fname" ? true : false}
                                    errorBorderColor='crimson'
                                />
                                {errorFeald === "fname" ? <Text color={"crimson"} fontSize={10} ml={2} mt={1}>Name is reqired*</Text> : null}
                            </FormControl>
                            <FormControl mt={1} id="mobile">
                                <FormLabel fontSize={12}>Mobile Number</FormLabel>
                                <Input
                                    onFocus={() => setErrorFeald("")}
                                    value={saveInfo.mobile} type="tel" onChange={e => setInfo(rest => {
                                    ...
                                    errorBorderColor='crimson'
                                />
                                {errorFeald === "mobile" ? <Text color={"crimson"} fontSize={10} ml={2} mt={1}>Phone number is reqired*</Text> : null}
                            </FormControl>
                            <FormControl mt={1} id="houseNo">
                                <FormLabel fontSize={12}>House Number</FormLabel>
                                <Input
                                    onFocus={() => setErrorFeald("")}
                                    value={saveInfo.houseNo} type="text" onChange={e => setInfo(rest => {
                                        return { ...rest, houseNo: e.target.value }
                                    })}
                                    isInvalid={errorFeald === "houseNo" ? true : false}
                                    errorBorderColor='crimson'
                                />
                                {errorFeald === "houseNo" ? <Text color={"crimson"} fontSize={10} ml={2} mt={1}>House number is reqired*</Text> : null}
                            </FormControl>
                            <FormControl mt={1} id="completeaddress">
                                <FormLabel fontSize={12}>Complete Address</FormLabel>
                                <Input
                                    onFocus={() => setErrorFeald("")}
                                    value={saveInfo.addressFromUser} type="text" onChange={e => setInfo(rest => {
                                        return { ...rest, addressFromUser: e.target.value }
                                    })}
                                    isInvalid={errorFeald === "addressFromUser" ? true : false}
                                    errorBorderColor='crimson'
                                />
                                {errorFeald === "addressFromUser" ? <Text color={"crimson"} fontSize={10} ml={2} mt={1}>Full address is reqired*</Text> : null}
                            </FormControl>
                        </Box>
                        <Box width={{ base: "100%", md: "56%" }} margin="auto" mt={{ base: 0, md: 5 }}>
                            <Button width={"100%"}
                                background="#0db616"
                                color="#fff"
                                fontSize={14}
...
function AddressType({ setInfo }) {
    ...
    useEffect(() => {
        setInfo(rest => { return { ...rest, addressType: value } });
    }, [value])
